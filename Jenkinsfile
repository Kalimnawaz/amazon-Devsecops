pipeline {
    ...
    stages {
        ...
        stage("Git Checkout") {
            steps {
                git branch: 'main', url: 'https://github.com/Kalimnawaz/amazon-Devsecops.git'
            }
        }
        ...
        stage("Build Docker Image") {
            steps {
                script {
                    env.IMAGE_TAG = "kalimnawaz/amazon:${BUILD_NUMBER}"
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'docker-cred', variable: 'dockerpwd')]) {
                        sh "docker login -u kalimnawaz -p ${dockerpwd}"
                        sh "docker tag amazon ${env.IMAGE_TAG}"
                        sh "docker push ${env.IMAGE_TAG}"

                        // Also push latest
                        sh "docker tag amazon kalimnawaz/amazon:latest"
                        sh "docker push kalimnawaz/amazon:latest"
                    }
                }
            }
        }
    ...
    always {
        script {
            ...
            emailext (
                ...
                """,
                to: 'kalim.nawaz20@gmail.com',
                from: 'kalim.nawaz20@gmail.com',
                mimeType: 'text/html',
